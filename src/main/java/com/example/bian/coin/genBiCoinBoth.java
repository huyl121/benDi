...
public class genBiCoinBoth {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        genBiCoinBoth genDan = new genBiCoinBoth();
        genDan.method(null);
    ...
    public void method(ThreadPoolExecutor threadPoolExecutor) throws InterruptedException {
        ...
        Map<String, Boolean> hasMap = hasOrder(biCoin_personInfoList.get(0));
        while (true) {
            try {
                diaoYongCount++;
...
//                PrivateConfig.printLog(newOrders.toJSONString());
                ...
                for (Object entryNew : newOrders) {
                    ...
                    if (!PrivateConfig.biCoin_symName.equals(symName)) {
                        continue;//指定跟单人
                    }
                    String msgId = entity.getString("msgId");
                    ...
                    String positionSide = PositionSide.BOTH.toString();
                    String side = "BUY";
                    if (labelSub.contains("卖出")) {
                        side = "SELL";
                    }
                    boolean reduceOnly = false;//手里没有单子时，只能买，不能卖，所以只能是false
                    if(hasMap.containsKey(symbol)){
                        if(hasMap.get(symbol)){
                            if(side.equals(OrderSide.SELL.toString())){
                                reduceOnly = true;
                            }
                        }else {
                            if(side.equals(OrderSide.BUY.toString())){
                                reduceOnly = true;
                            }
                        }
                    }
    ...
    public static Map<String, Boolean> hasOrder(JSONObject personInfo) throws InterruptedException {
        Map<String, Boolean> map = new HashMap<>();
        SyncRequestClient syncRequestClientTian = (SyncRequestClient) personInfo.get(PrivateConfig.syncRequestClient);
        ...
        for (Position position : positionListTian) {
            if (position.getPositionAmt().compareTo(ling) > 0) {
                map.put(position.getSymbol(), true);//正在做多
            }else if (position.getPositionAmt().compareTo(ling) < 0) {
                map.put(position.getSymbol(), false);//正在做空
            }
        }
        return map;
    }
