...
/*
* 这个主要是为了提高资金利用率，所以只跟一些高频的人
* 小于5分钟的单子不能太多
* 大于1小时的单子也不能太多
*
*
* 通过改变 genDans_genPortfolioId，统一跟单的人
* 从动态的配置文件中读取现在是否有单子，有单子的话，是谁的单子
*
*
* 第一次购买时，设置倍数，因为每个带单员的带单金额不同
*
* 不跟的情况：
* 持有单子量小时
* 跟单时，带单员已经处于盈利状态
...
public class genDans {
    ...
    public void method(String[] args) throws InterruptedException, IOException {
        PrivateConfig.printLog("genDans跟单开始时间为：" + PrivateConfig.getCurrentTime());
        int youOrder = 0;
        String count = "0";
        ...
        if (dynamicInfo != null) {
            ...
            if (StringUtils.isNotBlank(dynamicInfo.getString("count"))) {
                count = dynamicInfo.getString("count");
            }
        ...
        //设置倍数
        PrivateConfig.setStartBeiShu(PrivateConfig.genDans_personInfoList, startBeiShu);
        ...
        Callable callable1 = new Callable() {
            ...
            public String call() throws Exception {
                ...
                jianKong4.method(null, threadPoolExecutor, PrivateConfig.genDans_personInfoList);
                return "";
            }
        };
        ...
        int countErr = 0;
        int countErrLarge = 0;
        Map<String, Order> oldOrders = getOrders1(threadPoolExecutor, true, genDans_genPortfolioId, new BigDecimal(count));
...
//                PrivateConfig.printLog("执行1次：" + PrivateConfig.getCurrentTime());
                ...
                if (youOrder == 1) {
                    ...
                    newOrders = getOrders1(threadPoolExecutor, false, genDans_genPortfolioId, new BigDecimal(count));
                } else {
                    for (Object o : genDans_genPortfolioIds) {
                        ...
                        count = genPortfolioId.getString("count");
                        newOrders = getOrders1(threadPoolExecutor, false, genDans_genPortfolioId, new BigDecimal(count));
                        if (newOrders != null && !newOrders.isEmpty()) {
                            ...
                            if (laoShiMoney != null) {
                                ...
                                PrivateConfig.setBeiShu(threadPoolExecutor, laoShiMoney, genDans_personInfoList);
                                //把倍数记录到本地，供重启时使用
                                for (JSONObject personInfo : genDans_personInfoList) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (qingCang) {
                        ...
                        qingCang3.qingCang(PrivateConfig.genDans_personInfoList, entryOld.getValue().getSymbol(), getPositionSide(entryOld.getValue().getPositionSide(), entryOld.getValue().getOrigQty()));
                        //删除旧订单
                        iterator.remove();

                        hasOrderSet.remove(symbolPoSide);
                        if(xiaoCeShi){
                            SyncRequestClient syncRequestClient = ((SyncRequestClient) PrivateConfig.genDans_personInfoList.get(0).get(PrivateConfig.syncRequestClient));
                ...
                if (!orderListNew.isEmpty()) {
                    ...
                    //下单
                    for (JSONObject person : PrivateConfig.genDans_personInfoList) {
                        try {
...
//                    Thread.sleep(2000);//前面有跟单，歇2秒再跟
    ...
    public Map<String, Order> getOrders1(ThreadPoolExecutor threadPoolExecutor, boolean old, String genPortfolioId, BigDecimal count) throws InterruptedException {
        ...
        JSONArray jsonArray;
        if(old && !CollectionUtils.isEmpty(PrivateConfig.genDans_xianYou)){
             ...
             * */
            jsonArray = PrivateConfig.genDans_xianYou;
        }else {
...
//        System.out.println("订单：" + jsonArray.toJSONString());
        if(jsonArray != null){
            ...
            for (Object o : jsonArray) {
                ...
                PrivateConfig.gangGan(genDans_personInfoList, symbol, trade.getBigDecimal("leverage"));
                BigDecimal qty = trade.getBigDecimal(PrivateConfig.positionAmount);
//                BigDecimal entryPrice = trade.getBigDecimal("entryPrice");
                ...
                if(!hasOrderSet.contains(hasOrderFlag)){
                    BigDecimal unrealizedProfit = trade.getBigDecimal("unrealizedProfit");//当前亏损金额
                    BigDecimal notionalValue = trade.getBigDecimal("notionalValue");//持仓金额
                    BigDecimal peiLv = unrealizedProfit.divide(notionalValue, 5, BigDecimal.ROUND_HALF_UP);
                    if(peiLv.compareTo(new BigDecimal(PrivateConfig.genDans_unrealizedProfit)) > 0){
                        continue;
                    }
