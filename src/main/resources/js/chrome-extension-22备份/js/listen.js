...
$(function () {
    ...
    function task0311333() {
        ...
        if (alarm >= 60) {
            diaoYongCount++;
            alarm = 0
            $.get("http://localhost:9186/jianKong", {data: resultOld, diaoYongCount:diaoYongCount, time:1});
    ...
    function task0311333456() {
        ...
        var liArr = document.querySelectorAll('#app li')
        //如果没有登录成功，不执行下面的代码
        if(document.URL != "https://coolco.vip/realfirm/39/43"){
            return
        }
        var resultNew = '';
        var resultNewTemp = '';
        for (var i = 0; i < liArr.length; i++) {
            ...
            if(parseInt(money) < moneyMin){
                moneyMin = parseInt(money)
            }

            if(parseInt(money) > moneyMax){
                moneyMax = parseInt(money)
            }
            ...
            if(typeStr.indexOf(type) != -1 && parseInt(money) > 500){
                typeStr.splice(typeStr.indexOf(type), 1);
                $.get("http://localhost:9186/qingCang", {symbol: type});
            }
        ...
        if (resultOldTemp != resultNewTemp) {
            ...
            for (var j = 0; j < typeStrTemp.length; j++) {
                ...
                if (resultOldTemp.indexOf(typeStrTemp[j]) == -1) {
                    //从有变成无时，通知
                    $.get("http://localhost:9190/sendEmail", {data:typeStrTemp[j] + "这单结束了。最多赔了" + moneyMin + "最多挣了" + moneyMax});
                    moneyMin = -100
                    moneyMax = 100
